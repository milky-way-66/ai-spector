...
**You describe what you need in chat.** The **orchestrator** (parent agent) classifies intent, asks clarifying questions when needed, and **delegates** to a specialized **subagent worker** — one workflow per worker. Workers read briefs under [agents/](./agents/README.md); the parent does not inline heavy runbooks.
...
Unsure? Say **"help me approve"** or call **`workflow_route`** — orchestrator uses [skills/_skill-router.md](./skills/_skill-router.md), may ask one question, then spawns a worker from [agents/](./agents/README.md).
