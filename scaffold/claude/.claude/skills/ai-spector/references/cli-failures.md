...
| `docops init` / `migrate` / `migrate --repair` fails | Run `npx ai-spector docops guide --prompt` — follow target paths + tasks; gap-fill per `.docops/guide/guides/DOCOPS_MANUAL_FALLBACK.md` (never overwrite existing files) | `npx ai-spector docops status --json` when CLI works |
...
2. Run **`npx ai-spector docops guide --prompt`** — use target `docTypes` paths and ordered tasks (do not move docs unless user approves).
3. Open **`.docops/guide/guides/DOCOPS_MANUAL_FALLBACK.md`** for scaffold/template gap-fill rules.
