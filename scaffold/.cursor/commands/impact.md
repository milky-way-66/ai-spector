...
Scope regen after a change. **You describe what changed** — not a graph node id. The agent resolves the traceability seed, then runs `graph impact` ([_graph.md](./_graph.md), [_workflow.md](./_workflow.md)).
...
Use [_cli-failures.md](./_cli-failures.md). If resolve or `graph impact` fails, show CLI output and fix — do not invent regenerate lists.
