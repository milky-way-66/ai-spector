...
- **SPA** (`buildMode: spa`): one entrypoint, framework components per screen; honor `prototypeBypassAuth` and `previewUri` in `screen-map.json` (see `route-defaults.example.json` and `spa/vue/`).
