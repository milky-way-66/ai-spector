...
Use a **slash command** — it **overrides** skill matching for that turn. See [commands/README.md](./commands/README.md).

| Wrong route? | Use |
|--------------|-----|
| "generate detail design" → resolve-task | `/generate-detail-design` |
| incremental add → generate | `/resolve-task` |
| document sign-off → task approve | `/review` |
| resume stuck task | `/task` |
